...
def save_to_csv(place_name, category, address, score, reviewer_name, review_text, has_photo, star_rating, review_date, review_link):
    key = (place_name, address, reviewer_name)
    if key not in existing_keys:
        with open(filename, "a", newline="", encoding="utf-8-sig") as f:
            fieldnames = ["가게명", "업종", "주소", "총평점", "작성자", "리뷰내용", "사진유무", "별점", "작성시간", "리뷰 페이지"]
            ...
            print(f"✅ 저장 완료: {place_name} - {reviewer_name}")
        existing_keys.add(key)
    else:
        print(f"⚠️ 중복으로 저장 생략: {place_name} - {reviewer_name}")
...
try:
    ...
    search_input.send_keys("부산구덕 고봉민" + Keys.ENTER)  # ← 엔터를 눌러서 검색
    time.sleep(2)
    ...
    if more_btns:
        ...
        while True:
                ...
                for place_idx in range(len(li_elements)):
                    ...
                    try:
                        ...
                        if "HIDDEN" in score_class:
                            print(f"⚠️ 장소 {place_idx + 1}: 리뷰 없음 (score 클래스 → HIDDEN) → 건너뜀")
                            continue
                    ...
                    # 6. 리뷰 본문 추출 (더보기 클릭 포함)
                        ...
                        # 저장
                        save_to_csv(place_name, category, address, score, reviewer_name, review_text, has_photo, star_rating, review_date, review_link)
            ...
            if "disabled" in next_class:
                ...
                print("➡️ 다음 페이지 세트 이동")
                driver.execute_script("arguments[0].click();", next_btn)
                time.sleep(1.5)
    else:
        print("ℹ️ '더보기' 버튼 없음 → 현재 페이지 내 li만 순회")
